...
function GetaroundSection(): React.JSX.Element {
  ...
  return (
    <section id="getaround" className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm space-y-4">
      ...
      {/* URL Webhook Getaround */}
      <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4">
        <p className="text-xs font-semibold text-gray-700 mb-1">URL Webhook Getaround</p>
        <p className="text-xs text-gray-400 mb-2">
          Configurez cette URL dans votre espace Getaround pour recevoir les événements en temps réel
          (réservations, check-in/out, messages).
        </p>
        <div className="flex items-center gap-2">
          <code className="flex-1 rounded-lg bg-white border border-gray-200 px-3 py-2 text-xs font-mono text-gray-700 select-all">
            https://appli.sunanddrive.com/api/v1/webhooks/getaround
          </code>
          <button
            type="button"
            onClick={() => void navigator.clipboard.writeText('https://appli.sunanddrive.com/api/v1/webhooks/getaround')}
            className="shrink-0 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-gray-500 hover:text-gray-700 hover:bg-gray-50"
            title="Copier">
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
